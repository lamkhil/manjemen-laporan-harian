...
const empty = { name: '', nip: '', email: '', password: '', role: 'user', unit_kerja: '', jabatan: '' }
...
export default function UsersPage() {
  ...
  useEffect(() => { if (isAdmin) load() }, [search])
  ...
  function startEdit(u) { setEditing(u.id); setForm({ ...empty, ...u, password: '' }); setError('') }
    ...
    <div className="space-y-4">
      ...
      <div className="card overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading && <tr><td colSpan={6} className="p-3 text-slate-500">Memuat…</td></tr>}
            {!loading && items.length === 0 && <tr><td colSpan={6} className="p-3 text-slate-500">Tidak ada data.</td></tr>}
            {items.map((u) => (
