...
export default function ReportPrintPage() {
  ...
  const stats = useMemo(() => {
    if (!r) return null
    const items = r.items || []
    const violations = items.filter(i => i.category?.is_violation).length
    ...
    return { items, violations, locations, top, sorted, max, pkl, baliho, pct }
  }, [r])
  ...
  return (
    <div className="bg-slate-100 min-h-screen p-4 print:p-0 print:bg-white">
      <div className="no-print mb-3 max-w-4xl mx-auto flex gap-2 justify-end">
      ...
      <div className="bg-white max-w-4xl mx-auto shadow print:shadow-none p-8 print:p-6 text-slate-800" style={{ fontFamily: 'Arial, sans-serif' }}>
        ...
        <div className="flex items-center gap-4 border-b pb-3">
          ...
          <div className="flex-1 text-center">
            <h1 className="font-bold text-lg tracking-wide">{(r.title || 'LAPORAN PENGAWASAN KEWILAYAHAN').toUpperCase()}</h1>
            {r.kecamatan && <h2 className="font-bold text-base">KECAMATAN {r.kecamatan.toUpperCase()}</h2>}
            <div className="text-xs mt-1">{formatTanggal(r.report_date)}, (Shift {r.shift}){jam}</div>
          </div>
        </div>
        ...
        <div style={{ pageBreakBefore: 'always' }} className="mt-8 print:mt-0">
          <h3 className="text-center font-bold text-base mb-3">RINCIAN AKTIVITAS</h3>
