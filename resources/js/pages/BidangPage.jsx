...
export default function BidangPage() {
  ...
  async function load() {
    setLoading(true)
    try { const { data } = await api.get('/bidangs'); setItems(data) }
  ...
  async function save(e) {
    ...
    setError('')
    try {
      if (editing === 'new') await api.post('/bidangs', form)
      else await api.patch(`/bidangs/${editing}`, form)
      cancel(); load()
    } catch (err) { setError(err?.response?.data?.message || 'Gagal menyimpan.') }
  }

  async function remove(id) {
    if (!confirm('Hapus bidang ini? Semua jenis layanan di dalamnya juga akan terhapus.')) return
    try { await api.delete(`/bidangs/${id}`); load() }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold">Bidang DPMPTSP</h1>
          <p className="text-sm text-slate-500">Sekretariat, Penanaman Modal, PTSP, UPTSA, dll.</p>
        </div>
        {isAdmin && <button className="btn-primary" onClick={startNew}>+ Bidang</button>}
      </div>
        ...
        <table className="w-full text-sm min-w-[640px]">
          <thead className="bg-slate-50 text-slate-600">
            <tr>
              ...
              <th className="text-left p-3">Jenis Layanan</th>
            ...
            {items.map((it) => (
              <tr key={it.id} className="border-t border-slate-100">
                ...
                <td className="p-3 text-slate-600 text-xs">
                  {(it.jenis_layanans || []).length === 0 ? '-' : `${it.jenis_layanans.length} jenis`}
                </td>
