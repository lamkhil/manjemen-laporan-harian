...
const empty = { name: '', color: '#3b82f6', is_violation: false, is_service: false, is_active: true, sort_order: 0, description: '' }
        ...
        <form className="card p-4 space-y-3" onSubmit={save}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <label className="inline-flex items-center gap-2 text-sm">
              <input type="checkbox" checked={!!form.is_violation} onChange={(e) => setForm({ ...form, is_violation: e.target.checked })} /> Bisa input jumlah pelanggaran
            </label>
            <label className="inline-flex items-center gap-2 text-sm">
              <input type="checkbox" checked={!!form.is_service} onChange={(e) => setForm({ ...form, is_service: e.target.checked })} /> Kategori pelayanan (form lengkap)
            </label>
            <label className="inline-flex items-center gap-2 text-sm">
      ...
      <div className="card overflow-hidden">
        ...
        <table className="w-full text-sm min-w-[720px]">
          <thead className="bg-slate-50 text-slate-600">
            <tr><th className="text-left p-3">Nama</th><th className="p-3">Warna</th><th className="p-3">Input Pelanggaran</th><th className="p-3">Pelayanan</th><th className="p-3">Aktif</th><th className="p-3"></th></tr>
          </thead>
          <tbody>
            {loading && <tr><td className="p-3 text-slate-500" colSpan={6}>Memuat…</td></tr>}
            {!loading && items.length === 0 && <tr><td className="p-3 text-slate-500" colSpan={6}>Tidak ada data.</td></tr>}
            {items.map((c) => (
              <tr key={c.id} className="border-t border-slate-100">
                ...
                <td className="p-3 text-center">{c.is_service ? <span className="pill bg-sky-100 text-sky-700">YA</span> : '-'}</td>
