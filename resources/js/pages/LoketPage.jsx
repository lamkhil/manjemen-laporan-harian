...
const empty = { name: '', lokasi_id: '', description: '', is_active: true, sort_order: 0 }
...
export default function LoketPage() {
  ...
  function startEdit(it) { setEditing(it.id); setForm({ ...empty, ...it, lokasi_id: it.lokasi_id || '' }); setError('') }
  ...
  async function save(e) {
    ...
    setError('')
    try {
      const payload = { ...form, lokasi_id: form.lokasi_id || null }
      if (editing === 'new') await api.post('/lokets', payload)
      else await api.patch(`/lokets/${editing}`, payload)
      cancel(); load()
    } catch (err) {
  ...
  async function remove(id) {
    ...
    try { await api.delete(`/lokets/${id}`); load() }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold">Loket Pelayanan</h1>
          <p className="text-sm text-slate-500">Master data loket (cth. Loket Perizinan, Loket Pengaduan).</p>
        </div>
        {isAdmin && <button className="btn-primary" onClick={startNew}>+ Loket</button>}
      </div>
        ...
        <form className="card p-4 space-y-3" onSubmit={save}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select className="input" value={form.lokasi_id} onChange={(e) => setForm({ ...form, lokasi_id: e.target.value })}>
          ...
          <tbody>
            ...
            {items.map((it) => (
              <tr key={it.id} className="border-t border-slate-100">
                ...
                <td className="p-3 text-slate-600">{it.lokasi?.name || '-'}</td>
